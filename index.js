...
// ─── OTHER SERVERS DIRECTORY ───
// servers.json (in the repo root) lists other, separately-deployed copies
// of this bot (each its own Heroku app). Add a line to that file every
// time you deploy a new one via the "Deploy to Heroku" template button,
// and it will appear here automatically on the next restart.
const SERVERS_JSON_FILE = path.join(__dirname, 'servers.json');
function loadOtherServers() {
    try {
        if (fs.existsSync(SERVERS_JSON_FILE)) {
            const parsed = JSON.parse(fs.readFileSync(SERVERS_JSON_FILE, 'utf8'));
...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/') {
        ...
        const otherServers = loadOtherServers();
        const otherServersHtml = otherServers.length === 0
            ? `<div class="up-board-row"><span class="up-ic">&#127760;</span><span><b>No other servers listed</b><div class="up-sub">Add them in servers.json on GitHub</div></span></div>`
            : otherServers.map(s => `<a href="${s.url}" target="_blank" class="up-board-row" style="text-decoration:none;"><span class="up-ic">&#127760;</span><span><b>${s.name}</b><div class="up-sub">${s.url}</div></span><span class="up-check" style="color:#00ff88;">&#8250;</span></a>`).join('');
        ...
        const __userPanelHtml = `
            ...
            <body>
                <div class="up-wrap">
                    ...
                    <div class="up-board-card">
                        <h3>Other Servers (${otherServers.length})</h3>
                        ${otherServersHtml}
                    </div>
